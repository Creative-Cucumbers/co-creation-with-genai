...
- AI とプログラミングを学ぶ意義
- AI の定義と歴史、生成 AI ができること
- AI の基礎概念（機械学習・ディープラーニング・大規模言語モデル・ルールベースシステム）
- AI ワークショップ：学生生活における AI への期待と不安、生成 AI でしたいこと・疑問

> [!NOTE]
> [前回のノート「AI 基礎」](../1_ai_basic/)を要約してみよう

> 前回は、AI とプログラミングを学ぶ意義を考え、AI の定義と歴史、生成 AI ができることを学びました。機械学習・ディープラーニング・大規模言語モデルの関係や、ルールベースシステムの特徴と限界を確認し、ChatGPT に説明や質問への回答を求める体験もしました。ワークショップでは「生成 AI は自分たち学生の生活をどう変えるだろうか？」をテーマに、AI への期待と不安、生成 AI でしたいことや疑問を共有しました。
...
- [初等中等教育段階における生成 AI の利活用に関するガイドライン（Ver.2.0）](https://www.mext.go.jp/content/20241226-mxt_shuukyo02-000030823_001.pdf)
...
> 1. 上記リンクを開き、対象モデル（例: GPT-5.x）を選びます。
...
多くの単語は1つのトークンに対応しますが、そうでないものもあります：indivisible。
絵文字のようなUnicode文字は、その元となるバイトを含む複数のトークンに分割されることがあります：🤚🏾
よく隣り合って現れる文字の並びは、ひとまとまりにされることがあります：1234567890
 -->
...
> [!NOTE]
> 生成AIと利用時の注意点についてのレポート構成を考えてもらおう

#### レポートの生成 (5 分)

> [!NOTE]
> 生成AIと利用時の注意点についてのレポートの中身を記載してもらおう
...
> [!WARNING]
> 著作権等の「合法性」（既存の作品やキャラクターを再現していないか）に注意
> 偏見等の「社会的受容性」（性別・人種・職業などへの偏見を含んでいないか）に注意
...
1. **明確性 (Clarity)**: 目的や期待する出力を曖昧にせず、具体的に伝えることでモデルが誤解する余地を減らします。

   **Before**
   > 卒業制作のアイデアを考えて。

   **After**
   > インタラクティブアート作品の卒業制作アイデアを 3 つ、200 文字以内で提案して。

2. **制約の明示 (Constraints)**: フォーマットや条件を明示すると、用途に合った出力を得やすくなります。

   **Before**
   > 展示する作品のアイデアを提案して。

   **After**
   > 学生でも実現可能で、展示会場の制約を考慮した作品のアイデアを、箇条書きで提案して。

3. **役割付与 (Role)**: モデルに役割や視点を与えることで、回答のトーンや専門性をコントロールできます。

   **Before**
   > 作品への講評をお願いします。

   **After**
   > あなたは現代アートに詳しいキュレーターです。作品への講評をお願いします。

4. **手順分解と例示 (Step-by-step & Examples)**: 複雑なタスクは手順を分けて依頼し、期待する形式のサンプルを示すことで、推論の質を高められます。

   **Before**
   > 作品の制作プランを考えて。

   **After**
   > ① テーマ整理、② 作品構造の提案、③ 必要なリソースの提案の順に回答して。
   >
   > 例：① 薄れる記憶 → ② 消えていく文字 → ③ パソコン、モニター
